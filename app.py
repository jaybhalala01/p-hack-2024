...
def generate_video_stream(filename):
    ...
    while cap.isOpened():
        ...
        # Get the current list of number plate files
        current_number_plates = len(numberplate_detections)

        # Check if there are changes in the directory
        if current_number_plates != os.listdir(img_path):
            # Clear the directory
            ...
            # Save the new number plates
            for numberplate in numberplate_detections:
                x1, y1, x2, y2 = numberplate["bbox"]
                cv2.imwrite(
                    f"active_number_plates/number_plate_{time.time()}.jpg",
                    mask_frame[y1:y2, x1:x2],
                )
            
            for filename in os.listdir(img_path):
                file_path = "active_number_plates/"+filename
                licensePaths.append(file_path)
            print(licensePaths)            
...
def video_feed(filename):
    return Response(
        generate_video_stream(filename),
...
def track_data():
    with open("json/track.json") as f:
        data = json.load(f)
    return jsonify(data)
